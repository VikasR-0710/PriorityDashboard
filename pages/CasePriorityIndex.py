...
def ensure_audit_table_exists():
    try:
        ...
        cur.execute("""ALTER TABLE DBD_CASE_AUDIT_HISTORY
...
def sync_audit_history(df: pd.DataFrame):
    ...
    try:
        ...
        cur.execute(f"INSERT INTO DBD_CASE_AUDIT_HISTORY (CASE_NUMBER, SNAPSHOT_TIMESTAMP, CHANGE_TYPE, CHANGED_COLUMNS, OLD_STATE, NEW_STATE, DATA_HASH) SELECT CASE_NUMBER, SNAPSHOT_TIMESTAMP, CHANGE_TYPE, PARSE_JSON(CHANGED_COLUMNS_STR), PARSE_JSON(OLD_STATE_STR), PARSE_JSON(NEW_STATE_STR), DATA_HASH FROM {temp_table_name}")
        conn.commit()

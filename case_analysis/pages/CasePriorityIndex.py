...
def ensure_audit_table_exists():
    try:
        ...
        cur = conn.cursor()
        cur.execute("""CREATE TABLE IF NOT EXISTS CASE_AUDIT_HISTORY (
...
def _get_latest_states(sf_conn, case_numbers: list) -> dict:
    ...
    query = f"SELECT CASE_NUMBER, DATA_HASH, NEW_STATE FROM CASE_AUDIT_HISTORY WHERE CASE_NUMBER IN ({placeholders}) QUALIFY ROW_NUMBER() OVER (PARTITION BY CASE_NUMBER ORDER BY SNAPSHOT_TIMESTAMP DESC) = 1"
    cur = sf_conn.cursor()
...
def sync_audit_history(df: pd.DataFrame):
    ...
    try:
        ...
        cur.execute(f"INSERT INTO CASE_AUDIT_HISTORY (CASE_NUMBER, SNAPSHOT_TIMESTAMP, CHANGE_TYPE, CHANGED_COLUMNS, OLD_STATE, NEW_STATE, DATA_HASH) SELECT CASE_NUMBER, SNAPSHOT_TIMESTAMP, CHANGE_TYPE, PARSE_JSON(CHANGED_COLUMNS_STR), PARSE_JSON(OLD_STATE_STR), PARSE_JSON(NEW_STATE_STR), DATA_HASH FROM {temp_table_name}")
        conn.commit()
